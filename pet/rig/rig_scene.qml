...
Item {
    ...
    property var partsModel: []   // [{id,file,_url,source_figure,px_rect,pivot,z,sway{...}}]
    ...
    // 行走分量
    readonly property real walkRot: walking ? Math.sin(clock.t * 0.011) * 1.4 : 0
    readonly property real walkBob:
        walking ? -Math.abs(Math.sin(clock.t * 0.008)) * 2.2 : 0
