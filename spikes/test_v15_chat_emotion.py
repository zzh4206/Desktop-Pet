...
from pet.chat_emotion import ChatEmotionEngine, ConversationEmotionStore, FEATURE_DIM, LABELS
...
with tempfile.TemporaryDirectory() as d:
    ...
    # 20:00 未加载模型时安全降级为 sleepy。
    missing = ChatEmotionEngine(os.path.join(d, "missing.npz"))
    check("晚间缺模型回退 sleepy", missing.evaluate([], "20:00", now).label == "sleepy")
    check("日间缺模型回退 neutral", missing.evaluate([], "09:00", now).label == "neutral")
    ...
    result = engine.evaluate(store.recent_messages(now), "20:00", now)
    check("高置信模型结果覆盖夜间默认", result.label == "happy" and not result.used_fallback)
    store.mark_slot("20:00", now)
    check("每日时段只运行一次", store.due_slots(["09:00", "20:00"], now) == ["09:00"])
