...
| `Ctrl+Alt+P`（Windows） | 显示或隐藏聊天面板。 |
| `Ctrl+Alt+T`（Windows） | 强制解除“吃鼠标”状态。 |
...
- “吃鼠标”仅在用户空闲、非勿扰、非活跃视频内容等条件满足时触发，单次时长受限，并提供快捷键、托盘菜单和自动超时释放作为退出路径。
- 可能影响系统或数据的 AI 工具调用会先展示风险信息并要求确认；确认失败时默认拒绝执行。
