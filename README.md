...
- `chat_emotion`：本地聊天情绪开关、每日兜底时段（默认 `22:00`）和夜间表情时长。每条用户消息仅在检测到高置信、非中性情绪时立即换表情；即时情绪保持到下一条明确情绪消息，避免横跳；22:00 没有明确情绪时显示困倦。仅在本机保留最近 48 小时的用户消息，可从托盘“聊天情绪设置”修改时段。
...
- “吃鼠标”仅在用户空闲、非勿扰、非活跃视频内容、非全屏演示（v0.14.11 起含到达点复查）等条件满足时触发，单次时长受限，并提供快捷键、托盘菜单和自动超时释放作为退出路径。
